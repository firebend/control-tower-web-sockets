...
describe('HubConnectionRealTimeConnection', () => {
  ...
  it('registers reconnected callback', () => {
    ...
    expect(hubConnection.onreconnected).toHaveBeenCalledWith(
      expect.any(Function),
    );
  });

  it('registers close callback', () => {
    // eslint-disable-next-line @typescript-eslint/no-empty-function
    realTimeConnection.onClose(() => {});

    expect(hubConnection.onclose).toHaveBeenCalledWith(expect.any(Function));
  });
});
